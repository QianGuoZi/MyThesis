...
\begin{itemize}
    \item \textbf{SWTS}\cite{SWTS}：每个作业的综合得分由以下四个因素加权组合：关键路径长度（CPL，与作业预计运行时间成反比）、任务并行度（TPS，与任务节点数 $K_j$ 正相关）、数据局部性评分（DLS，与总带宽需求 $\sum b_v^{\min}$ 成反比）以及资源匹配度（计算节点资源与任务需求的匹配程度）。此外，作业的优先级 $w_j$ 也被纳入评分体系。每次调度时，选择综合得分最高的作业进行映射。
