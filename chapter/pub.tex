...
\begin{table}
	\centering{}%
	\pubfont 
	\begin{longtable}{|>{\centering}m{0.5cm}|>{\centering}m{2.0cm}|>{\centering}m{2.8cm}|>{\centering}m{2.5cm}|>{\centering}m{2.2cm}|>{\centering}m{2.0cm}|>{\centering}m{1.0cm}|}
		\hline 
		\textbf{序号} & \textbf{作者（全体作者，按顺序排列）} & \textbf{题目} & \textbf{发表或投稿刊物名称、级别} & \textbf{发表的卷期、年月、页码} & \textbf{与学位论文哪一部分（章、节）相关} &\textbf{被索引收录情况}\tabularnewline
		\hline 
		1& 杨磊，梁华倩 & 《CES：面向分布式学习任务的边缘协同任务调度系统》 & 《计算机研究与发展》（CCF-T1类期刊） & - & 第三章、第四章 & 拟投稿 \tabularnewline
		\hline 
		% 2	 & 							&  	 &   &  &  & \tabularnewline
		% \hline 
	\end{longtable}
