...
\subsection{边缘环境}
边缘环境包含物理节点集$\mathcal{P}$与物理链路集$\mathcal{R}$。一个边缘环境包含多个物理节点和多条物理链路。每个物理节点$p \in \mathcal{P}$有固定的CPU资源$C_p^{\text{CPU}}$和内存资源$R_p^{\text{RAM}}$。连接两个物理节点$p_i$和$p_j$的物理链路集合记为$\mathcal{R}_{p_i}^{p_j}=\{r_a,r_b,\dots\}$。每条物理链路$r \in \mathcal{R}$是有向的，且有固定的带宽资源$\text{BW}_r^{\text{total}}$。

\subsection{作业}
对于当前待调度作业，其任务节点集合记为$\mathcal{N}$，任务链路集合记为$\mathcal{V}$。一个作业中包含多个任务节点和多条任务链路。每个任务节点$n \in \mathcal{N}$有所需的CPU资源$\text{CPU}^\mathcal{N}(n)$和内存资源$\text{RAM}^\mathcal{N}(n)$。任务网络建模成P2P网络，相连的两个任务节点$n_i$和$n_j$之间仅存在一条链路，记为$\mathcal{V}_{n_i}^{n_j}=v$。每条链路$v \in \mathcal{V}$是有向的，其所需的带宽资源是弹性的，最小带宽为$\text{BW}_v^{\min}$，最大带宽为$\text{BW}_v^{\max}$，动态分配变量$\text{BW}_v^{\text{alloc}}$表示实际分配的带宽，且$\text{BW}_v^{\min} \leq \text{BW}_v^{\text{alloc}} \leq \text{BW}_v^{\max}$。当两个任务节点映射到同一个物理节点时，他们之间的链路将不再消耗物理带宽资源，即可以最大地满足用户的带宽请求。带宽分配变量$\text{BW}_v^{\text{alloc}}$由调度器动态决策，在满足$\text{BW}_v^{\min} \leq \text{BW}_v^{\text{alloc}} \leq \text{BW}_v^{\max}$前提下优化全局目标。
